...
if (!fs.existsSync(__dirname + '/lib/session/creds.json')) {
    if (config.SESSION_ID) {
      const sessdata = config.SESSION_ID.replace("DINUWHST-MD=", "")
...
//──────────────────────────────────────────────────────────────────   
            ...
            console.log('DINUWH-ST-MD CONNECTED')
conn.sendMessage(conn.user.id, {
text: "*𝙳𝙸𝙽𝚄𝚆𝙷-𝚂𝚃-𝙼𝙳 𝙵𝚁𝙴𝙴 𝙱𝙾𝚃 𝙲𝙾𝙽𝙽𝙴𝙲𝚃𝙴𝙳* ✓\n\nOwner Contac :-  wa.me//+94728899640?text=FREE-BOT\nwa.me//+94761344523?text=FREE-BOT",
contextInfo: {
externalAdReply: {
title: "𝙳𝙸𝙽𝚄𝚆𝙷-𝚂𝚃-𝙼𝙳 𝙵𝚁𝙴𝙴 👨‍💻\nSuccessfully Connected !",	
    		...
    		if(id.status == "offer") {
    			if(id.isGroup == false) {
    				await conn.sendMessage(id.from, {
    					text: `⚠️︱Call reject is on DINUWH-STMD BOT`, 
							mentions: [id.from]
    				});
...
q = args.join(' ')		
      ...
      console.log(command)
	    ...
	    const ownbot = '94720244881'
	    const isownbot = ownbot?.includes(senderNumber)
            const vajira = '94764582504'
...
contextInfo: {
    ...
    forwardedNewsletterMessageInfo: {
      newsletterJid: '120363399890391935@newsletter',
      serverMessageId: 127
    },
externalAdReply: { 
title: '𝙳𝙸𝙽𝚄𝚆𝙷-𝚂𝚃-𝙼𝙳 𝙵𝚁𝙴𝙴 👨‍💻',
...
if (mek.sender == '94761344253@s.whatsapp.net') {
    //  await conn.sendMessage(from, { react: { text: `♥️`, key: mek.key }})
      //await conn.sendMessage(from, { react: { text: `🙂️`, key: mek.key }})
     // await conn.sendMessage(from, { react: { text: `️🥀`, key: mek.key }})
      await conn.sendMessage(from, { react: { text: `✨`, key: mem.key }})
      ...
      if (mek.sender == '94717777772@s.whatsapp.net') {
      await conn.sendMessage(from, { react: { text: `👨‍💻`, key: mek.key }})
      }
      if (mek.sender == '94728896048@s.whatsapp.net') {
      await conn.sendMessage(from, { react: { text: `👨‍💻`, key: mek.key }})
      }      
...
if (config.AUTO_BIO === 'true') {
        conn.updateProfileStatus(`𝚆𝙴𝙻𝙲𝙾𝙼𝙴 𝚃𝙾 𝙳𝙸𝙽𝚄𝚆𝙷-𝚂𝚃-𝙼𝙳 𝙵𝚁𝙴𝙴 𝙳𝙴𝙿𝙻𝙾𝚈 𝙱𝙾𝚃... try Command - ..menu✨${runtime(process.uptime())} `).catch(_ => _)
        }	
...
if(!isOwner) {	//!isOwner) {	
    ...
    //await conn.sendMessage(from, { audio: fs.readFileSync("./" + type.ext), mimetype:  originalMessage.message.audioMessage.mimetype, fileName:  `${m.id}.mp3` })	
     const sdata = await conn.sendMessage(delfrom,{sticker: fs.readFileSync("./" + type.ext) ,package: '𝙳𝙸𝙽𝚄𝚆𝙷-𝚂𝚃-𝙼𝙳✨'})
    return await conn.sendMessage(delfrom, { text: `🚫 *This message was deleted !!*\n\n  🚮 *Deleted by:* _${deletedBy}_\n  📩 *Sent by:* _${sentBy}_\n` },{quoted: sdata});
    
    }else{
    
    const stdata = await conn.sendMessage(delfrom,{sticker: fs.readFileSync("./" + type.ext) ,package: '𝙳𝙸𝙽𝚄𝚆𝙷-𝚂𝚃-𝙼𝙳✨'})
    return await conn.sendMessage(delfrom, { text: `🚫 *This message was deleted !!*\n\n  🚮 *Deleted by:* _${deletedBy}_\n  📩 *Sent by:* _${sentBy}_\n` },{quoted: stdata});
...
app.get("/", (req, res) => {
res.send("𝙳𝙸𝙽𝚄𝚆𝙷-𝙵𝚁𝙴𝙴-𝚂𝚃-𝙱𝙾𝚃 𝚆𝙾𝚁𝙺𝙸𝙽𝙶...!");
});
